...
-- qa's repro confirmed Postgres 16 accepts it, and this project's local reference Postgres (used
-- throughout dev/qa verification) is 16 — plus it is a single atomic statement, so (unlike
-- dropping and recreating) there is never a window where a concurrent write to `public.tunables`
-- could land while this trigger is absent.
create or replace trigger tunables_0_validate_update
