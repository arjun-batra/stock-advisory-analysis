...
**2026-07-30 — `DEEP-007` targeted for resolution via INC-12 (Decision #37); built, qa-tested, reviewer
NOT CLEAR pending one fix cycle.** `DEEP-007` (kill-switch in-flight-run boundary) was excluded from the
round above pending an unresolved user trade-off (abort an in-flight run vs. rescope FR24's wording); the
user chose the former. `FR24` was reworded (§5.10) and a new `FR35` added for the mid-run-abort
...
INC-12 and qa passed all 9 literal ACs (`docs/test-report.md`) — but reviewer's Pass 28 diff-scoped audit
returned **NOT CLEAR**: two new majors, **REV-116** (a fifth, pre-checkpoint-1 irreversible write —
`config.write_tunables_cache_if_fetched()` — found by tracing past the four named checkpoints; DEEP-007 is
therefore **not yet resolved**) and **REV-117** (`sql/kill_switch_abort_log.sql`'s `REVOKE` omits
`TRUNCATE`, blocking that file from being applied live). Neither blocks INC-12's own qa-passed code from
being on `main`; both block treating DEEP-007 as closed. One fix cycle is in progress: dev owns REV-116's
code move/gate and REV-117's one-line SQL fix; tech-lead owns the design sentences these findings
falsified — §13.6.5's `REVOKE` sample is corrected as of this update (REV-117), §13.1's "no irreversible
action possible in that window" claim (REV-116) is corrected once dev's code fix lands and the exact
resolution (move vs. gate) is known. Design lives in `docs/design/operational-controls.md` §13.6 and
...
| `docs/design/increment-plan.md` **(INC-3–INC-10 all IMPLEMENTED, INC-8/9/10 reviewer-CLEAR Passes 24/25/27; INC-11 approved, not yet executed; INC-12 dev-built + qa-PASS, reviewer Pass 28 NOT CLEAR — REV-116/REV-117 open, fix cycle in progress)** | The project plan: INC-3 through INC-12, design pointers, file lists, dev-self-verifiable acceptance criteria. Split out of `design.md` 2026-07-28. | n/a (project plan, not a numbered design section) |
...
| `docs/design/operational-controls.md` **(§13.1–§13.5/§14 IMPLEMENTED, INC-3/INC-4; §13.6 dev-built + qa-PASS, INC-12, reviewer Pass 28 NOT CLEAR — REV-116/REV-117 open, fix cycle in progress)** | Kill-switch (dispatch-layer enforcement, audit trail, monitor pause-awareness, in-flight boundary checks) and AI provider abstraction (interface, LiteLLM-vs-hand-rolled decision) | §13–§14 |
...
| FR35 (mid-run pause-abort classification, Decision #38, INC-12) | **Dev-built, qa-tested PASS; reviewer Pass 28 verdict NOT CLEAR** — REV-116/REV-117 open (neither is an FR35 defect; both are elsewhere in INC-12's scope), fix cycle in progress. `operational-controls.md` §13.6.3. |
