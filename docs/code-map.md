...
stale). Map for "does the code still match this" — for rationale read `docs/design.md` / `docs/design/
*.md`.
...
- `run_hourly.py`, `run_discovery.py`, `publish_prices.py` — thin entry points, no business logic; each
  ...
  tunables cache back (REV-116 open: this write currently precedes its own boundary check).
...
**built, not yet applied live**, REV-117). INC-10 fix-round, each additive: `tunables_validate_
