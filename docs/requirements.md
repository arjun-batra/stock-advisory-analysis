...
- **FR30** — The portal includes a tunables editor covering a curated subset of `scripts/config.py` values
  ...
  fetches them from this table at run start, falling back to the current hardcoded Python defaults if the
  fetch fails (same fail-safe posture already used elsewhere, e.g. the AI call path). The write path uses
  the portal's authenticated Supabase session, gated by an admin-scoped RLS write policy — the same
  authorization mechanism already used for FR28/29 watchlist/holdings CRUD — so no GitHub PAT or other
  broad-scope credential is used or stored by the portal (see NFR6). This supersedes the original
  GitHub-Actions-Variables-proxy mechanism recorded in Decision #24; see Decision #27.
...
- **NFR6 — Admin portal security:** Write access (watchlist, holdings, tunables, kill-switch) requires
  ...
  Supabase RLS write policy, not solely by the client-side auth check, consistent with NFR3's "secrets
  never in code" posture and `scripts/config.py`'s existing "nothing sensitive hardcoded" convention.
...
> keys (falling back to the hardcoded Python defaults if the fetch fails); the portal writes to this table
> directly under an admin-scoped RLS policy, with no GitHub PAT or proxy involved. All other tunables in
> the tables above remain GitHub-Actions-Variable/code-default only; the portal does not expose them and
> this reversal does not affect them.
