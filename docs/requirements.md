...
> back to a repo-committed cache file (`config/tunables_cache.json`) holding the last successfully-fetched
> value per key, not a fixed hardcoded literal. The cache file is seeded on day one with the then-current
> hardcoded defaults, and is updated (diff-checked, committed only if changed) on every successful
> Supabase fetch — the same commit-on-change mechanism already used by
> `.github/workflows/publish-prices.yml` for `pages/prices.json`. Write-ownership (which workflow(s) write
> back to the cache vs. read it read-only) is **settled in `design.md` as `hourly-watchlist.yml` being the
> sole writer (Decision #28), pending Arjun's reconfirmation per Decision #29** — reviewer's Pass 11 audit
> (REV-040) surfaced a race and a privilege-increase issue with that shape and recommended switching the
> writer role to `publish-prices.yml` instead. All other tunables in the tables above remain
...
| 2026-07-12 | Corrected kill-switch fail-open/fail-closed wording (FR30, NFR5, §11) to match actual code behavior (fail-open only on a truly unset/empty Variable; any explicitly-set-but-wrong value, including typos, fails closed). Section later retired and removed outright 2026-07-16 — see below and git history. | Resolved REV-001 (major) / BUG-001 doc-vs-code contradiction via user's chosen Option B (correct the docs, not the code); superseded by later retirement. |
