...
> `.github/workflows/publish-prices.yml` for `pages/prices.json`. Which workflow(s) own writing back to the
> cache vs. reading it read-only is a design-level call left to tech-lead (see Decision #28). All other
> tunables in the tables above remain GitHub-Actions-Variable/code-default only; the portal does not expose
> them and this reversal/refinement does not affect them.
