...
- **NFR8 — Admin portal UI/UX quality (responsive & modern) (change request, 2026-07-31; Decision #39):**
  The admin portal's existing five screens — login (FR27), watchlist/holdings CRUD (FR28/FR29), the
  tunables editor (FR30), the track-record view (FR31), and the kill-switch toggle (FR32) — must be
  visually modernized and fully responsive, with **no change to any FR27–FR32 functional behavior, the
  `is_admin()`/RLS authorization model, or any data model/schema**. This is a UI/UX-only requirement.
  - **Responsive (all five screens, all three device classes):** at phone, tablet, and desktop viewport
  ...
  - **Modern visual design, via approved mockups:** the designer researches modern admin/SaaS UI patterns
    ...
    all five screens in `docs/ux-spec.md`; the user selects one direction before any implementation
    begins (this selection is the GATE — dev may not start until it is made); the delivered UI must match
    the approved direction's mockups. This mockups-before-implementation sequencing is an explicit,
    non-skippable condition of this NFR, not merely a process preference.
  ...
  - **No functional regression:** qa's acceptance criteria for the increment(s) implementing this NFR
    must confirm zero behavioral regression to FR27–FR32 (login/auth, CRUD, tunables validation,
    track-record data, kill-switch mechanics) alongside the visual/responsive changes — this NFR adds a
    presentation-layer bar on top of those FRs, it does not modify them.
