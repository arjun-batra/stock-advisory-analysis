...
| 39 | Admin Portal UI/UX modernization — change request received 2026-07-31, scope-clarification pending, no FR/NFR added yet | Arjun's CR: the admin portal (FR27–FR32) UI "is not screen size responsive" and "looks very basic" — modernize it. Explicitly **UI/UX enhancement only**: no change to FR27–FR32's functional behavior, `is_admin()`/RLS authorization model, or any table schema. Arjun explicitly requires mockups/wireframes to be presented and approved before implementation proceeds. Per the no-inference rule, the following are undecided and are **not guessed**: which screens are in scope (login, watchlist/holdings CRUD, tunables editor, track-record view, kill-switch header/toggle — all vs. a prioritized subset), target breakpoints/device classes, what "modern" means in testable terms (a specific component library/design system, brand/color constraints, or free rein for designer), whether an accessibility bar (e.g. WCAG level) is expected, and whether this is one increment or phased/prioritized across screens. Routed to the user as clarifying questions; **not resolved here**. Once answered, pm expects to frame the result as a new NFR (a testable responsive/visual-quality bar layered over the existing FR27–FR32 screens) rather than a new functional FR, since no new capability is being added — pending confirmation the answers don't imply otherwise. Gate: designer's `docs/ux-spec.md` (mockups/wireframes) must be produced and approved by the user before dev touches any code — this gate is explicit in the CR and must not be skipped, and does not depend on whether the final ID is an NFR or FR. | Same-class handling as DEEP-007 (Decision #37): a genuine, user-facing trade-off (scope/breakpoints/definition of "modern") is a decision for the user, not pm's or tech-lead's to infer or default. Recording the CR's receipt and open questions as a decision (not a finalized requirement) keeps the record honest about what has and hasn't been decided, consistent with this project's established pattern for open items awaiting user input. |
