...
## Pass 18 — 2026-07-29 (INC-6 admin portal tunables editor, FR30 — diff-scoped audit) — ARCHIVED
2026-07-29 at Pass 19's close

Archived at Pass 19's close, per `CLAUDE.md`'s doc-hygiene rule (full finding text is in git history at
the Pass-18 commit / prior revisions of `docs/review-log.md`). Pass 18 was INC-6's diff-scoped audit
(FR30, and NFR6 via FR30's write path): complete requirement→design→code→test traceability with one
doc-baseline gap (REV-087), zero `[SCOPE-CREEP]`, the core hardcoding-removal mechanism (10 keys off
env-var/hardcoded control) genuinely implemented with no new `[HARDCODED]`, no new `[BLOAT]`, and one new
gating `[SECURITY]` minor (REV-086: `sql/admin_portal_tunables.sql` missing a narrow `revoke insert,
delete, truncate` statement — the `tunables` table's sibling of REV-081's `admin_allowlist` fix, found
before first live application rather than after). Verdict was **NOT CLEAR**, held on REV-086 alone, per
this project's own Pass-16/REV-081 precedent for the identical finding class. Three non-blocking minors
also opened (REV-087 requirements-baseline gap, REV-088 test-report internal inconsistency, REV-089 stale
DRAFT status headers across four design docs), plus three carried items independently re-verified and
closed this pass (REV-075, REV-084, REV-085 — see their own disposition below, unchanged from the prior
revision of this entry).

**Closing disposition (Pass 18, 2026-07-29) — carried items, each independently re-verified against
current file content, not against any fix commit's own claim:**

- **REV-075** `[BLOAT]` (comment accuracy) minor — RESOLVED. `scripts/config.py:257-258` now reads "The
  effective values are logged at call setup (`ai_judge.judge_batch`)"; independently confirmed
  `ai_judge.py:264-266` genuinely prints all three named values in one `[ai_judge] call config:` line.
- **REV-084** `[DESIGN-GAP]` minor — RESOLVED. The "NOT CLEAR pending REV-081" qualifier is gone from all
  four cited spots (`docs/design.md`, `docs/design/increment-plan.md`, `docs/design/admin-portal.md`) —
  all now correctly state "reviewer Pass 17 verdict is CLEAR." Repo-wide grep confirmed zero remaining
  hits of the stale phrase.
- **REV-085** `[BLOAT]`/staleness minor — RESOLVED. `docs/handoff.md` no longer contains the stale AC8
  "explicitly pending" bullet — the file was rewritten in full for INC-6's own handoff, superseding the
  INC-5-era content (preserved in git history).

**Closing disposition (Pass 19, 2026-07-29) — REV-086/087/088/089, each independently re-verified against
current file content, not against the fix commits' claims, per Pass 19's explicit brief:**

- **REV-086** `[SECURITY]` minor — RESOLVED. `sql/admin_portal_tunables.sql:25` now reads exactly
  `revoke insert, delete, truncate on public.tunables from public, anon, authenticated;`, placed
  immediately after `alter table public.tunables enable row level security;` (`:24`). Independently
  confirmed: (1) the statement deliberately omits UPDATE and SELECT — both legitimately granted to
  `authenticated` via the `admin_write_tunables` policy (`:54-57`, `for select, update to authenticated`),
  the mechanism the whole FR30 feature depends on; revoking either would silently break the portal's save
  path since RLS filters rows but does not itself confer the underlying table privilege; (2) it is
  materially narrower than, not a copy of, `admin_allowlist`'s REV-081 REVOKE
  (`sql/admin_portal_rls.sql:17`, which includes UPDATE — correct there because no `authenticated`-role
  write path to that table is legitimate, wrong here); (3) an inline comment (`:26-32`) explains the
  divergence explicitly so a future editor doesn't "helpfully" align the two lists. Live application not
  independently re-verified (no live-DB tool bound to this session, same evidentiary status as REV-070 and
  REV-081's live half) — the migration has not yet been applied to the live project (INC-6 hasn't shipped),
  so there is nothing live to check yet; the orchestrator applies it post-clearance, same process as
  INC-5's `sql/admin_portal_rls.sql`.
- **REV-087** `[REQUIREMENTS-GAP]` minor — RESOLVED. `docs/requirements.md:351-352` now carries both rows
  (`TUNABLES_FETCH_TIMEOUT_MS` default `5000`, `SKIP_TUNABLES_FETCH` default `false`) in the §10
  Core-system table. Changelog entry at `:418` (2026-07-29) records the fix, names REV-087, and notes the
  archival of the oldest entry to hold the cap. Changelog independently recounted at exactly 10 live
  entries (`:409-418`) — cap honoured.
- **REV-088** `[TEST-GAP]`-adjacent doc-hygiene minor — RESOLVED. `docs/test-report.md:136`'s AC14 row now
  reads "PASS — all 3 entry points, including BUG-003 fix"; the Verdict paragraph (`:170-179`) now reads
  "PASS. BUG-003 found this session and FIXED by dev (commit `799cd35`); no bugs remain open" with no
  remaining "PARTIAL"/"open bug" language — consistent with the Gaps and Open Bugs sections, which already
  said FIXED.
- **REV-089** `[DESIGN-GAP]` minor — RESOLVED as scoped. All four named files now correctly state INC-6 is
  dev-built/qa-tested and explicitly not-yet-reviewer-clear: `docs/design/increment-plan.md:1,3-18`
  (title, status note, and the `### INC-6` heading itself, `:161`) and the matching `**Status: IMPLEMENTED**
  ... reviewer Pass 18 verdict NOT CLEAR pending REV-086 fix in progress` framing in
  `docs/design/admin-portal-tunables.md:10-12`, `docs/design/tunables-fallback.md:10-12`, and
  `docs/design/tunables-workflow-writeback.md:11-14`. INC-7 correctly untouched/DRAFT throughout
  (`increment-plan.md:17-18,286`). REV-079's secondary residual (AC5 baseline wording,
  `increment-plan.md:117-118`, still naming only `non-functional-ops.md` §9) remains open — tech-lead
  explicitly left it out of this fix's scope; carried forward, not treated as gating INC-6's clearance (an
  INC-3/INC-4-era wording residual, not an INC-6 defect). New residual found independently this pass — see
  REV-090 in `docs/review-log.md` Pass 19 (`docs/design.md` itself, the master index every agent reads
  first, was outside REV-089's scope and still states INC-6 is DRAFT/not-yet-built in multiple spots,
  directly contradicting the four files this fix corrected).
...
derivation; FR30/FR11/FR29; DEEP-005+DEEP-006 closure) — ARCHIVED at Pass 27's close (2026-07-30)

Archived here in full per `CLAUDE.md`'s doc-hygiene rule now that REV-112 and REV-113 — the two findings
this pass's original text below still needed closed — are both independently re-verified RESOLVED at
Pass 27 (`docs/review-log.md`). REV-111 (status markers) and REV-114 (no permanent test of either new
SQL trigger's live behaviour) remain open and are carried forward, unarchived, in the live log — same
precedent as Pass 14/REV-075 and Pass 16/REV-084-085. Pass 26's own verdict was **CLEAR** (zero blockers,
one non-blocking major — REV-113 — at the time it was written).
...
### Original findings — Pass 26 (both now RESOLVED — see closing dispositions below)

**REV-111 — `[DESIGN-GAP]` — minor — owner: tech-lead. STILL OPEN, not archived, unchanged by this
verification — see the live log's carried-forward list.** INC-10's status markers are stale the moment
Pass 26's verdict landed — the same propagation pattern this log has flagged repeatedly before a merge
event (REV-073, REV-079, REV-084, REV-090, REV-093/094, REV-108, REV-110), now a fourth time in the same
handful of files. Location: `docs/design.md:45,49` ("INC-9, INC-10, and INC-11 are approved ... not yet
built"), `:81` (`non-functional-ops.md` index row, "STALE fix-round additions ... INC-9/INC-10, pending
dev"), `:84` (`admin-portal.md` index row, "§16.3 has a STALE fix-round addition, INC-10, pending dev"),
`:85` (`admin-portal-tunables.md` index row, same phrasing), `:211` (FR2/FR11 coverage row, "STALE pending
...
release (apply-time instruction). RESOLVED — see closing disposition below.** `sql/admin_portal_tunables.sql`'s
new corrective `UPDATE` (`:97-99`) is bundled into a migration file whose earlier statements (`create table`,
`create policy`, the seed `insert`) are **not** re-runnable against the live project — `create table
public.tunables` alone errors `relation "tunables" already exists` on any second apply. `docs/runbook.md`
§2.3 documents this exact file as a one-time, apply-once-in-order migration ("Deployed as part of INC-6"),
and §5's own stated convention for a post-hoc data correction is "write a corrective SQL script and apply
it manually" — i.e. a **separate** script, which is the pattern the two brand-new files in this same round
(`tunables_validate_trigger.sql`, `holdings_currency_derivation.sql`) correctly follow. As packaged, a
release engineer who follows §2.3's normal apply instruction (or a tool like `supabase db push`, which
tracks already-applied migrations by filename and would skip this file entirely on a re-run) has no clear
path to actually landing the description correction against the live row — they must know, from
`handoff.md` alone, to extract and run only lines 97-99 by hand. The content of the `UPDATE` itself is
correct and genuinely idempotent in isolation (confirmed: scoped to one column, one `WHERE key =
'ALERTS_ENABLED'`, safe to run any number of times) — this finding is about the packaging, not the SQL.
Not a blocker: worst case is the correction silently fails to apply and the seed row's description stays
stale (a cosmetic/documentation gap, not a functional or security one) rather than any corruption.
Suggested fix: either a new, separate `sql/admin_portal_tunables_alerts_enabled_description_fix.sql` (one
`UPDATE` statement, matching the two new trigger files' own additive-file convention), or, at minimum, an
explicit INC-11-prerequisite note (parallel to qa's PG14+ note) telling release to apply only the trailing
`UPDATE`, not the whole file.

**Closing disposition (Pass 27, 2026-07-30, `docs/review-log.md`) — RESOLVED, independently re-verified
against current file content, not accepted on dev's/qa's account.** `sql/admin_portal_tunables.sql:87-93`
now carries only a one-line pointer comment where the trailing `update` used to be; lines 1-86 (the
already-applied `create table`/trigger/policies/seed `insert`) are unchanged. The new, separate
`sql/admin_portal_tunables_alerts_enabled_description_fix.sql` carries the relocated `UPDATE` verbatim, as
its own additive, idempotent, re-runnable file — matching the convention `tunables_validate_trigger.sql`/
`holdings_currency_derivation.sql` established this same increment, exactly the fix this finding suggested.
qa independently re-ran BUG-008's own re-runnability standard against a local Postgres 16 scratch database
seeded with the pre-DEEP-005 stale description and confirmed the new file both corrects it and is
idempotent on a second apply — reviewer read that evidence and the file's own header (which documents the
same reasoning) directly rather than accepting the "holds" claim on account. Full evidence trail: Pass 27,
`docs/review-log.md`.

**REV-113 — `[DESIGN-GAP]` — major, non-blocking — owner: tech-lead (prompt/`_ticker_block` design).
RESOLVED — see closing disposition below.** `scripts/ai_judge.py:101-106`'s `_ticker_block` still renders
`Cost basis: {position['cost_basis']} {position['currency']}, Current price: {data['price']}` verbatim even
when `build_position` has suppressed `pl_pct` to `None` for a currency mismatch — `current price` carries
no currency label at all in that line, and nothing in `prompts/batch_system_prompt.txt` tells the model not
to compute its own gain/loss estimate from two adjacent, unlabeled, differently-denominated numbers.
DEEP-006's own suggested-fix text said a `build_position` guard "would stop a bad row from reaching the
prompt at all" — the shipped fix stops the *derived figure*, not the *underlying row*, from reaching the
prompt. This reproduces DEEP-006's exact "quietly wrong answer that looks right" risk class via model
inference instead of system arithmetic, on the system's sole real-money advice surface. Rated major
(matching DEEP-005/006's own severity for the same risk class) but explicitly non-blocking for INC-10, for
the same reason DEEP-006 itself was accepted as latent: this is the already-narrowed residual case
(`watchlist.market` wrong for the ticker) layered under a case the live watchlist doesn't currently exercise
(zero held positions), and it needs its own design decision (e.g., omit `Cost basis`/`Current price`
entirely from the block on a detected mismatch, or label `Current price` with its own currency and add an
explicit "these two figures are not comparable" note), which is out of dev's scope to invent unilaterally.
Suggested owner sequencing: same shape as DEEP-005/006 — tech-lead records the design decision, dev
implements, qa tests.

**Closing disposition (Pass 27, 2026-07-30, `docs/review-log.md`) — RESOLVED, independently re-verified
against current code, tests, and the whole rendered prompt block, not accepted on dev's/qa's account.**
`scripts/ai_judge.py:101-116`'s held-position branch now checks `position["currency_mismatched"]` — a new
field `state.build_position` (`scripts/state.py:164-202`) exposes as the single source of truth for the
same mismatch condition it already computes for `pl_pct` suppression, so `ai_judge.py` does not re-derive
the condition a second time. On a mismatch, **both** raw figures (`cost_basis` and `price`) are omitted
from the held-position line — not just the derived `pl_pct` — replaced with a sentence naming both
currencies and instructing the model not to compute or state a gain/loss for the position; `Shares` is
kept (not currency-denominated, no leakage risk). Independently traced the whole rendered block, not just
the one changed line, for any other route a raw cost-basis figure could reach the model: `_ticker_block` is
the sole per-item prompt renderer (`judge_batch`'s `blocks` comprehension at `:356` is its only caller), and
`judge_batch` is the sole prompt-construction path in the codebase — `run_discovery.py` always calls it with
`position: None` (candidates are never held), so the mismatch branch is reachable only via `run_hourly.py`'s
`state.build_position(holdings.get(ticker), data)` call, built from the same `data` object passed alongside
it into `judge_batch`, with no intervening mutation — confirmed no second construction path, no stale-cache
path, and no way for `currency_mismatched` and the actual currencies it was computed from to diverge, since
both are derived from and read off the same `data`/`position` pair in the same call. The current price is
not lost — it still appears, correctly labeled with the fundamentals currency, in the unrelated
"Price/volume" line every ticker (held or not) already carries. Four new `tests/test_ai_judge.py` tests
were read and independently confirmed to test the fix, not incidental behavior: one confirms the omission
wording, one walks the **whole** rendered block (not just the changed line) and confirms the raw
`50.0`/`50` cost-basis figure survives nowhere else and the remaining price is uniquely, correctly labeled,
one locks in the specific pre-fix line shape as absent (named so a future regression is caught by name), and
one confirms the agreeing-currency case is byte-identical to the pre-fix rendering (the fix only branches on
an actual mismatch). Three existing `test_state.py` tests were extended to assert `currency_mismatched`
directly (not just `pl_pct`'s side effect), including the "missing fundamentals is unknown, not mismatch"
case — confirming the flag itself, not merely its downstream effect, is correct across all three cases
(match/mismatch/unknown). **Judgment on the design choice (omission over labeling):** agree with dev's
call. Labeling both figures with their own currencies (the reviewer's other suggested shape) still leaves
two numbers adjacent that *look* subtractable/divisible to a model that half-follows an instruction not to
compute one; omitting both raw figures removes the arithmetic opportunity structurally rather than trusting
an instruction to suppress it, which is the same posture this project has taken everywhere else a model
input could be misused (e.g. the "headlines are data, not instructions" injection guard). The instruction
line still names both currencies and states plainly that a mismatch exists, so the model is not left
guessing that the position is held — only the two comparable-looking numbers are withheld. **Judgment on
the `currency_mismatched` contract widening:** sound. `state.py` remains the single owner of the FR11
invariant-detection logic (no duplicated condition in `ai_judge.py`), the new field is additive to
`build_position`'s return dict (no existing test asserts the full key-set, confirmed non-breaking), and the
flag cannot diverge from the actual currencies given the call graph traced above — there is exactly one
producer and one same-call consumer of the pair, with no path for a stale or mismatched re-pairing. One
adjacent design-doc staleness surfaced independently, not part of this finding's original scope, logged
fresh in the live log as REV-115 (owner tech-lead): `docs/design/components.md` §4.4's one-line prompt-
content summary ("shares/cost-basis/price/P&L for held") is no longer unconditionally true after this fix.
Full evidence trail: Pass 27, `docs/review-log.md`.

**REV-114 — `[TEST-GAP]` — minor — owner: qa. STILL OPEN, not archived, unchanged by this fix cycle — see
the live log's carried-forward list.** Neither new SQL trigger's actual runtime behaviour (rejecting a bad
write, overriding a submitted currency) is exercised by a permanent, CI-re-run test — only by dev's and
qa's own ephemeral local-Postgres sessions (real, and independently corroborated across two fix cycles, but
not repeatable automatically). The **static** shape of both files (idempotent creation syntax, no
redefinition of existing objects, presence of a validation branch per key) **is** permanently tested
(`tunables_static.test.ts`, `static_source_checks.test.ts`) — this finding is specifically about the
...
**Verdict at Pass 26 (as originally written): CLEAR** — see live log Pass 27 for the current open-items
list (REV-111, REV-114 carried; REV-115 new).

---

## Pass 28 — 2026-07-30 (INC-12 diff-scoped audit — kill-switch in-flight boundary checks + mid-run-abort
classification; FR24, FR35; DEEP-007 closure attempt) — ARCHIVED at Pass 29's close (2026-07-30)

Archived here in full per `CLAUDE.md`'s doc-hygiene rule now that REV-116, REV-117, REV-118, and REV-119 —
the findings that held INC-12 NOT CLEAR and DEEP-007 open at this pass — are all independently re-verified
RESOLVED at Pass 29 (`docs/review-log.md`), against a genuine fix cycle (dev's `run_hourly.py`/
`sql/kill_switch_abort_log.sql` fix, qa's two new load-bearing regression tests, tech-lead's §13.1/§13.6.2
design corrections and §0 rule #13). REV-120 (FR35 wording tension, owner pm) and REV-121 (status-marker
propagation, owner tech-lead — resolved as scoped by the same fix cycle, see Pass 29) are tracked in the
live log's current open-items list, not here. Pass 28's own verdict was **NOT CLEAR** (three new majors:
REV-116, REV-117, REV-118).
...
commit path is reachable before FR24 checkpoint 1 ever executes. RESOLVED 2026-07-30 (Pass 29,
`docs/review-log.md`) — see closing disposition there.**
...
same gap class already found and fixed four times in this codebase. RESOLVED 2026-07-30 (Pass 29,
`docs/review-log.md`) — see closing disposition there.**
...
new file and one earlier one. RESOLVED 2026-07-30 (Pass 29, `docs/review-log.md`, original scope only —
see closing disposition there for a fresh residual found in the same file) — see closing disposition there.**
...
checkpoint 1's hard-crash failure mode on a Supabase-unreachable `is_paused()` call. RESOLVED 2026-07-30
(Pass 29, `docs/review-log.md`) — see closing disposition there.**
...
`outcomes["no-read"]` (a documented fail-safe placeholder, not a real verdict). STILL OPEN, not archived,
unchanged by this fix cycle — see the live log's carried-forward list.**
...
REV-073/079/084/090/093-094/108/110/111/115. RESOLVED-AS-SCOPED 2026-07-30 (Pass 29,
`docs/review-log.md`) — see closing disposition there for the next instance of the same pattern this
...
## DEEP-007 — original finding + status history (archived 2026-07-30 at Pass 29's close)

**Original finding (2026-07-29, pre-INC-12), `[REQUIREMENTS-GAP]`, minor at the time (re-scoped to major at
Pass 28).** The kill-switch stops future *dispatches*, not the system. A run already executing when the
toggle flips completes in full — Yahoo fetches, the batched AI call, real pushes, and a commit to `main` —
while the portal badge already reads `PAUSED`.
...
30-minute cycle — hence minor, not major (at the time this was first logged).
...
those four boundaries — independently re-verified, see Pass 28 above. **Not fully closed:** Pass 28 traced
the code beyond the four named checkpoints and found a fifth, unguarded irreversible-action path —
`run_hourly.py`'s `config.write_tunables_cache_if_fetched()` call, which executed unconditionally before
checkpoint 1 was ever reached and could still produce a real `contents: write` commit to `main` during a
pause. Logged as REV-116 (major), Pass 28.

**Closing disposition (Pass 29, 2026-07-30, `docs/review-log.md`) — RESOLVED, independently re-verified
against current code, not accepted on dev's/qa's account.** Re-traced all three entry points
(`run_hourly.py`, `run_discovery.py`, `publish_prices.py`) from the first line of `main()` forward,
independently of dev's and qa's own claims that only `run_hourly.py` had the defect and that the other two
were already clean. Confirmed directly: `run_discovery.main()` goes `require_secrets()` → `state.client()`
(no I/O beyond client-object construction — `create_client()` performs no network call by itself) →
`notify.get_notifier()` (pure object construction, no I/O) → checkpoint 1, with nothing irreversible in
between; `publish_prices.main()` goes `require_secrets()` → `state.client()` → `state.get_watchlist()` (a
read) → a Yahoo-fetch loop (reads only, no external mutation) → checkpoint 4 immediately before the
`pages/prices.json` write, and checkpoint 1 is correctly out of scope for this file per FR24's own text.
`run_hourly.main()` now reads `config.require_secrets("SUPABASE_URL", "SUPABASE_SECRET_KEY")` →
`sb = state.client()` → checkpoint 1's `is_paused()` read, with nothing else preceding it — the tunables-
cache write (`config.write_tunables_cache_if_fetched()`) and the market-gate computation both now sit
...
## REV-151/REV-152/REV-153/REV-155 — INC-15 Tickers-merge fix-cycle findings + closure (archived 2026-08-01
at Pass 38's close)

**Original findings (2026-08-01, Pass 37, INC-15 diff-scoped audit — Tickers merge, nav defect fix,
FR36/FR37/FR38).**

**REV-151 — `[SECURITY]` — major.** Both new RPCs (`set_ticker_holding_status`, `delete_ticker` in
`sql/tickers_screen_rpc.sql`) were missing the `revoke execute ... from public, anon, authenticated`
statement that precedes the `grant execute ... to authenticated` line for every other `SECURITY DEFINER` RPC
in this codebase. PostgreSQL grants `EXECUTE` on a newly created function to `PUBLIC` by default; this
codebase has repeatedly needed and applied the belt-and-suspenders fix for exactly this default
(`sql/kill_switch.sql:115`, `sql/scheduler_pgcron.sql:92-93,156`, `sql/phase5_monitoring.sql:300-303,340`).
Absent the revoke, `anon` (a fully unauthenticated caller) could still invoke either RPC over PostgREST's
`/rpc/` endpoint using nothing but the public anon key — not currently exploitable to bypass authorization
(`is_admin()`'s runtime check inside the function body independently blocks any such call from mutating
data, since an anon-key request carries no `email` JWT claim resolving inside `admin_allowlist`) but a
genuine least-privilege gap, identical in kind to REV-081 (`admin_allowlist` TRUNCATE grant)/REV-086
(`tunables` TRUNCATE grant)/REV-117 (`kill_switch_abort_log` TRUNCATE grant). The design's own SQL block
(`docs/design/admin-portal.md:744,756` at the time) had the identical omission — dev correctly implemented
the design's literal block, so the gap originated in the design, not dev's transcription. Blocked live
application of `sql/tickers_screen_rpc.sql` only, not the `admin-portal/` code merge. Owner: tech-lead
(`admin-portal.md` §16.11.5's SQL block) + dev (the two-line SQL fix).

**REV-152 — `[CODE-GAP]` — major.** `TickerEditModal.tsx`'s held→watch-only confirmation path silently
discarded any unsaved `market`/`type` edit made in the same modal session. Traced `doSave()`/
`handleSaveClick()`/`confirmSwitchToWatchOnly()` line-by-line: `handleSaveClick` intercepted before
`doSave()` ran whenever `ticker.status === "held" && form.status === "watch-only"`, so `doSave()` (the only
place issuing `supabase.from("watchlist").update({market, type})`) was never called on this path;
`confirmSwitchToWatchOnly()` issued only the `set_ticker_holding_status` RPC, no `market`/`type` write of any
kind. Concretely: an admin changing `market` (e.g. US→TSX) and switching status to watch-only in the same
sitting would have the status transition and holdings deletion succeed, but the market edit silently
dropped — never sent to the database, no error shown, modal closed as if the save fully succeeded. Not
covered by any of the 14 finalized ACs (AC8 only tests a pure status-only transition) and not exercised by
dev's 54-assertion script or qa's 31 supplementary checks. Did not falsify any of the 14 literal ACs and did
not block merging the diff's 14-AC-scoped functionality, but was a genuine silent-data-loss correctness gap.
Owner: dev (route `market`/`type` through the held→watch-only confirm path too).

**REV-153 — `[STRUCTURE]` — major.** `docs/code-map.md`'s `admin-portal/` inventory was stale against
INC-15's actual structure change — same class of gap as REV-145 (Pass 34) and REV-143/118 before it. Still
listed `(app)/watchlist|holdings|tunables|track-record/` and `components/` `AuthGuard.tsx`,
`KillSwitchToggle.tsx`, `NavToggle.tsx` (with `NavToggle.tsx`'s description still describing the pre-INC-15
"CSS forces the panel open at desktop widths" mechanism, now false), with no mention of the new `tickers/`
route, the deleted `watchlist`/`holdings` routes, `TickerEditModal.tsx`, or `sql/tickers_screen_rpc.sql`.
`docs/design/increment-plan.md`'s own INC-15 entry had already anticipated this exact fix ("tech-lead
refreshes `docs/code-map.md` before this increment's merge commit"). Not an application-code defect — the
specific `CLAUDE.md` git-workflow precondition for merging a structure-changing increment, same framing as
Pass 34/REV-145. Owner: tech-lead, a mechanical multi-line update (route list, component list +
`NavToggle.tsx`'s description, `sql/` file list).

**REV-155 — `[DESIGN-GAP]` — minor.** `layout.tsx`'s omission from both `admin-portal.md` §16.11.6's and
`increment-plan.md`'s formal allow-lists was a real doc-accuracy gap, not a scope violation — both
documents' own requirement-coverage rows assigned FR38's title rename to this increment, so the file's
inclusion was correctly authorized by the very same documents that failed to list it in their file-allow-list
tables. The actual change (`layout.tsx`'s `<title>` metadata string, one line) was confirmed to be exactly
what FR38 asked for and nothing else. Owner: tech-lead (add `layout.tsx` to §16.11.6's/`increment-plan.md`'s
allow-list tables).

**Closing disposition (Pass 38, 2026-08-01, `docs/review-log.md`) — all four RESOLVED, independently
re-verified against current code and docs, not accepted on the fix commits' messages alone.** Dev's fix
commit `d9702aa` (REV-151/REV-152) and tech-lead's fix commit `157b805` (REV-153, REV-155, plus the
admin-portal.md §16.11.5 SQL-block grant-pattern sync) were confirmed correctly scoped: dev's commit
confined to `sql/tickers_screen_rpc.sql`, `admin-portal/components/TickerEditModal.tsx`,
`tests/admin_portal/static_source_checks.test.ts`, and `docs/handoff.md`; tech-lead's commit confined to
`docs/code-map.md`, `docs/design/admin-portal.md`, `docs/design/increment-plan.md` — no drift into each
other's territory, cross-checked via qa's independent `docs/test-report.md` "INC-15 fix cycle 1" entry (no
direct shell `git show` access either pass). `sql/tickers_screen_rpc.sql:53-54,66-67` now carries the
revoke-then-grant pair for both functions, byte-for-byte matching `sql/kill_switch.sql:115`'s pattern — REV-151
RESOLVED. `TickerEditModal.tsx`'s shared `applyMarketTypeEdit()` helper is now called from both `doSave()`
and `confirmSwitchToWatchOnly()` before their respective writes, confirmed via direct read plus two new
committed regression tests in `tests/admin_portal/static_source_checks.test.ts` (verified meaningful — both
fail against the pre-fix `d6e6ad7` content and pass against the fix, per qa's independent trace) — REV-152
RESOLVED. `docs/code-map.md`'s `admin-portal/`/`sql/` inventories now name `tickers/page.tsx`,
`TickerEditModal.tsx`, and `sql/tickers_screen_rpc.sql`, with `watchlist`/`holdings` removed and
`NavToggle.tsx`'s stale description corrected — REV-153 RESOLVED. `docs/design/admin-portal.md` §16.11.6's
and `docs/design/increment-plan.md`'s allow-list tables now both name `admin-portal/app/layout.tsx` — REV-155
RESOLVED. INC-15 cleared to merge to `main` on this pass with zero blockers and zero open majors; the two
carried, non-blocking minors (REV-149, REV-154) remain open, neither dropped nor falsely resolved. Full
re-verification detail is in `docs/review-log.md` Pass 37 (original audit) and Pass 38 (closing
verification).
