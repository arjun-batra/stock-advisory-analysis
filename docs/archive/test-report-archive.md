...
bugs below filed) — see the earlier archived entry above.
...
## INC-14 — Admin portal visual fidelity fix (NFR8 conformance, corrects INC-13) — 2026-08-01 (archived — superseded by INC-15's Tickers-merge/nav-fix pass)
...
- Given the "no functional change" boundary NFR8/INC-13/INC-14 all state explicitly, and that "status" is
  ...
  increment's sign-off given the schema/architecture constraints above. **Resolved by INC-15**: watchlist
  and holdings merged into one Tickers screen, so this ambiguity no longer applies to any live screen.
...
## INC-15 (original pass) — Tickers merge + nav defect fix + horizontal-scroll tier (FR36/FR37/FR38, amended NFR8) — 2026-08-01
